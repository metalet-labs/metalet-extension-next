...
const prettyTaskGas = computed(() => {
  ...
  return `≈ ${calcBalance(taskGas.value, 8, 'BTC')}`
})
...
watch(
  () => finalToken1Amount.value < DUST_UTXO_VALUE,
  (n) => {
    emit('returnBecameNegative', n)
  },
  { immediate: true }
)
...
<template>
  <div class="flex flex-col gap-2 p-4 text-xs lg:text-sm">
    ...
    <div class="flex items-center justify-between text-xs lg:text-sm" v-if="showFinal">
      ...
      <div class="flex items-center gap-3">
        <div class="text-right text-xs text-zinc-500 lg:text-sm" v-if="taskGas && finalToken1Amount >= DUST_UTXO_VALUE">
