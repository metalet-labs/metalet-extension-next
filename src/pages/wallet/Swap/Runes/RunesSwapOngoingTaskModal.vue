...
import { RuneAsset } from '@/data/assets'
...
import { useOngoingTaskQuery } from '@/queries/runes/swap'
      ...
      <div class="mt-4 rounded-3xl border border-zinc-700 p-2">
        ...
        <div class="rounded-2xl bg-black px-4 py-2" v-if="extendedTask">
          <div class="flex flex-col items-center gap-2 text-sm" v-if="extendedTask.type === 'swapToRune'">
            ...
            <div class="flex items-center gap-2">
              <RunesTokenIcon :symbol="rune.symbol" class="size-6 rounded-full" v-if="extendedTask.toToken" />
              <span>
                {{ calcBalance(extendedTask.toAmount, rune.decimal, rune.symbol) }}
              </span>
            </div>
          </div>
          ...
          <div class="flex flex-col items-center gap-2 text-sm" v-if="extendedTask.type === 'swapToBtc'">
            <div class="flex items-center gap-2">
              <RunesTokenIcon :symbol="rune.symbol" class="size-6 rounded-full" v-if="extendedTask.fromToken" />
              <span>
                {{ calcBalance(extendedTask.fromAmount, rune.decimal, rune.symbol) }}
              </span>
            </div>
          ...
          <div class="" v-else-if="extendedTask.type === 'add'">
            <div class="flex flex-col items-center gap-2 text-sm">
              ...
              <div class="flex items-center gap-2">
                <RunesTokenIcon :symbol="rune.symbol" class="size-6 rounded-full" />
                <span>
                  {{ calcBalance(extendedTask.toAmount, rune.decimal, rune.symbol) }}
                </span>
              </div>
