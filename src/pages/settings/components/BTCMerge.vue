...
const merge = async () => {
  if (feeRate.value && utxos.value) {
    ...
    for (let i = 0; i < utxos.value.length; i += Number(mergeNum.value)) {
      const splitUtxos = utxos.value.slice(i, i + Number(mergeNum.value))
      const transaction = await wallet.merge(feeRate.value, splitUtxos)
...
<template>
  <div class="min-h-full flex flex-col">
    ...
    <div class="space-y-4 grow mt-4">
      ...
      <div class="h-80 overflow-y-auto nicer-scrollbar -mr-4 pr-4" v-if="transactions.length">
        <table class="min-w-full table-auto">
          ...
          <tbody>
            <tr v-for="(transaction, index) in transactions" :key="index">
              <td class="border px-4 py-2 text-xs">{{ prettifyTxId(transaction.txId, 4) }}</td>
