...
export function privateKeyFromWIF(wifString: string, network?: bitcoin.Network | bitcoin.Network[]): string {
  ...
  if (Array(network)) {
    ...
    if (version !== (network as bitcoin.Network).wif) throw new Error('Invalid network version')
  }
...
export function signBtc(
  ...
  if (!fakePrivateKey) {
    fakePrivateKey = private2Wif(
      base.fromHex('853fd8960ff34838208d662ecd3b9f8cf413e13e0f74f95e554f8089f5058db0'),
      network
    )
  }

  if (changeOnly) {
    let { inputAmount, outputAmount, virtualSize } = calculateTxSize(
      inputs,
      outputs,
      changeAddress,
      fakePrivateKey,
  ...
  if (utxoTx.memo && utxoTx.memoPos == 0) {
    txBuild.addOutput(
      ...
      base.toHex(
        bscript.compile(
          ([OPS.OP_RETURN] as Stack).concat(
            base.isHexString(utxoTx.memo) ? base.fromHex(utxoTx.memo) : Buffer.from(base.toUtf8(utxoTx.memo))
          )
        )
      )
    )
...
export function signBch(
  ...
  let { inputAmount, outputAmount, virtualSize } = calculateBchTxSize(
    inputs,
    outputs,
    changeAddress,
    fakePrivateKey,
...
function calculateTxSize(
  ...
  if (memo && pos == 0) {
    preTxBuild.addOutput(
      '',
      0,
      base.toHex(
        bscript.compile(
          ([OPS.OP_RETURN] as Stack).concat(
            base.isHexString(memo) ? base.fromHex(memo) : Buffer.from(base.toUtf8(memo))
          )
        )
      )
    )
  ...
  // change
  if (inputAmount - outputAmount > dustSize.valueOf()) {
...
export function estimateBtcFee(utxoTx: utxoTx, network?: bitcoin.Network) {
  ...
  // calc tx size
  const fakePrivateKey = private2Wif(
    base.fromHex('853fd8960ff34838208d662ecd3b9f8cf413e13e0f74f95e554f8089f5058db0'),
    network
  )
  let { virtualSize } = calculateTxSize(
    inputs,
    outputs,
    utxoTx.address,
    fakePrivateKey,
