...
export function signPsbtWithKeyPathAndScriptPathBatch(
  psbtHexs: string[],
  privateKey: string,
  network?: Network,
...
export function signPsbtWithKeyPathAndScriptPath(
  psbtStr: string,
  privateKey: string,
  network?: Network,
  opts: signPsbtOptions = {}
...
export function signPsbtWithKeyPathAndScriptPathImpl(
  psbt: Psbt,
  privateKey: string,
  network?: Network,
  autoFinalized?: boolean,
  signInputs?: toSignInput[]
) {
  ...
  for (let i = 0; i < psbt.inputCount; i++) {
    ...
    if (isTaprootInput(input)) {
      ...
      if (input.tapLeafScript && input.tapLeafScript?.length > 0 && !input.tapMerkleRoot) {
        ...
        signer.publicKey = Buffer.from(
          taproot.taprootTweakPubkey(toXOnly(wif2Public(privateKey, network)), input.tapMerkleRoot)[0]
        )
      }
    } else {
      signer.needTweak = false
...
export function mergeSignedBuyingPsbt(signedBuyingPsbt: string, signedListingPsbts: string[]) {
  ...
  signedListingPsbts.forEach((signedListingPsbt, i) => {
    ...
    buyerSignedPsbt.data.inputs[nftIndex + i] = sellerSignedPsbt.data.inputs[SELLER_INDEX]
  })
...
export function generateMPCSignedListingPSBT(
  psbtBase64: string,
  pubKeyHex: string,
  signature: string,
  network?: Network
...
export function generateMPCUnsignedBuyingPSBT(
  psbtBase64: string,
  pubKeyHex: string,
  network?: Network,
  batchSize: number = 1
...
export function generateMPCSignedBuyingTx(
  psbtBase64: string,
  pubKeyHex: string,
